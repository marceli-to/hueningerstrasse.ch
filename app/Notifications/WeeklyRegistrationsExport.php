...
class WeeklyRegistrationsExport extends Notification
{
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $mail = (new MailMessage)
            ->subject(config('app.name').' – Neue Anfragen ('.$period.')')
    ...
    private function buildCsv(): string
    {
        ...
        fputcsv($handle, ['Datum', 'Vorname', 'Name', 'Strasse/Nr.', 'PLZ/Ort', 'E-Mail', 'Telefon', 'Interesse', 'Nachricht'], ';', '"', '');
        ...
        foreach ($this->registrations as $registration) {
            fputcsv($handle, [
                ...
                $registration->email,
                $registration->phone,
                collect($registration->apartment_sizes)->map(fn ($s) => $s === 'gewerbe' ? 'Gewerbefläche' : $s.'-Zi.')->implode(', '),
